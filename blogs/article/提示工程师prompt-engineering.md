...
给LLM指定角色、任务、背景（输入信息）、要求（期待输出）。例如：`你是一个资深数学教师，请解释下面题目，需要让只会四则运算的小学生能够听懂，请尽量通俗，可使用比喻的方式。题目【某数学题】`

> 角色并不是一定要指定的，在使用ChatGPT时，我们并不需要给它安排一个角色或头衔，因为这并不会影响到它的表现。即使你不给它安排一个角色，它也能根据提示词为你提供相应的结果。但是如果你需要模拟一个"两个人"的交互场景，就需要给ChatGPT安排一个头衔，让它扮演某个角色。
