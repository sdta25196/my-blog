...
[ASCII](https://www.runoob.com/w3cnote/ascii.html)，最基础的编码，是由美国人定义的，他们用1字节（8位）来定义他们使用的所有字符
...
所以unicode也叫万国码、单一码，目前最新版本为2022年9月13号发布的第15版。[unicode.org](https://www.unicode.org/versions/Unicode15.0.0/)

Unicode中的字符由`0000`到`10FFFF`（16进制）的单一数值范围组成。从`0000`开始，为每个符号指定一个编码，叫做**码点**，比如码点`0061`就是`a`, 

```js
  u+0061 = a
```

为了方便，Unicode使用平面（plane）来区分码点，最大值为`10FFFF`，对应二进制有21位，按照2^16一组划分，每个平面可以存放65536个（2^16）字符。一共划分17个平面。

plane0，也就是最前面的65536个字符位，称为基本平面（缩写BMP），它的码点范围是从0一直到2^16-1，写成16进制就是从U+0000到U+FFFF。所有最常见的字符都放在这个平面，这是Unicode最先定义和公布的一个平面。其他16个平面被称为辅助平面(SMP)

plane1，补充多语言平面(SMP)。存放历史文字、符号、数字

plane2，辅助表意平面(SIP)。从平面中的第一个代码点开始，专门用于编码额外的统一CJK字符。 （CJK characters => Chinese, Japanese, and Korean）

plane3，第三象形平面(TIP)。是专门用于附加统一编码的CJK字符，最著名的是篆书。 

其他平面统称：Other Planes。

**unicode仅仅是规定一个字典（a == 0061），并没有规定编码的具体实现方式，于是就有了 `utf-8`、`utf16`等编码规则**

### utf32

最直观的编码方法是，每个码点使用四个字节表示，字节内容一一对应码点。这种编码方法就叫做UTF-32。比如，码点0就用四个字节的0表示，码点`597D`就在前面加两个字节的0。
...
> utf32使用4个字节表示一个字符，完全对应unicode编码，这样查询效率虽然高O(1)，但是非常浪费空间，相比ASCII编码文件大了四倍

UTF-32的优点在于，转换规则简单直观，查找效率高。缺点在于浪费空间，同样内容的英语文本，它会比ASCII编码大四倍。

这个缺点很致命，导致实际上没有人使用这种编码方法。
...
JavaScript没有使用上述编码中的任何一种，JavaScript用的是**UCS-2**

### UCS-2编码

互联网还没出现的年代，曾经有两个团队，不约而同想搞统一字符集。一个是1988年成立的Unicode团队，另一个是1989年成立的UCS团队。等到他们发现了对方的存在，很快就达成一致：世界上不需要两套统一字符集。

1991年10月，两个团队决定合并字符集。从今以后只发布一套字符集，就是Unicode，并且修订此前发布的字符集，UCS的码点将与Unicode完全一致。
...
两者的关系简单说，**就是UCS-2整合进了UTF-16。**所以，现在只有UTF-16，没有UCS-2。

捋一下时间线：

* 1995年5月，Brendan Eich用了10天设计了JavaScript语言。
* 1995年10月，第一个JavaScript解释引擎（SpiderMonkey）问世。
* 1996年11月，Netscape正式向ECMA提交语言标准（整个过程详见《JavaScript诞生记》）。
* 而1996年7月UTF-16才发布，当时的Netscape公司没有其他选择，只有UCS-2一种编码方法可用。

### JavaScript字符函数的局限性

由于JavaScript只能处理UCS-2编码，造成所有字符在这门语言中都是2个字节，如果是4个字节的字符，会当作两个**双字节**的字符处理。

JavaScript的字符函数都受到这一点的影响，无法返回正确结果。

以字符`𝌆`为例，它的UTF-16编码是4个字节的`0xD834 DF06`。问题就来了，4个字节的编码不属于UCS-2，JavaScript不认识，只会把它看作单独的两个字符`U+D834`和`U+DF06`。
...
```js
  for(let i=0;i<'𝌆'.length;i++){
      console.log('𝌆'.charCodeAt(i).toString(16))
  }
```
...
在html中直接使用unicode的话，就需要把unicode转换为html实体：

unicode 与 [html实体](https://www.w3school.com.cn/charsets/ref_html_symbols.asp) 转换：`\u0021 === &#x0021; === &#33; === !`
